...
async function installPartner(){
  ...
  async function redeem(raw){const token=String(raw||'').trim();if(!/^[0-9a-f-]{36}$/i.test(token))return $('pointsRedeemMsg').textContent='Invalid redeem code.';if(!confirm('Confirm that you are handing this gift to the customer now?'))return;try{stop();$('pointsRedeemMsg').textContent='Confirming collection…';const r=await rpc('partner_redeem_points_gift',{p_redeem_token:token});const x=r?.[0];$('pointsRedeemMsg').className='success';$('pointsRedeemMsg').textContent=`Collected ✓ ${x?.customer_name||''} · ${x?.gift_title||''}`;$('pointsRedeemCode').value=''}catch(e){$('pointsRedeemMsg').className='error';const code=String(e?.message||'');$('pointsRedeemMsg').textContent=code.includes('ALREADY_COLLECTED')?'This gift was already collected.':code.includes('INVALID_REDEEM_CODE')?'Invalid gift code. Please check the customer QR.':code.includes('WRONG_PARTNER')?'This gift belongs to another Partner.':code.includes('NOT_APPROVED')?'This gift is not approved for collection yet.':(code||'Could not confirm this gift.')}}
...
async function installAdmin(){
  ...
  panel.innerHTML=`<div class="card"><div class="row"><div><h2>WENIK Points</h2><div class="muted">Current conversion and redemption control.</div></div><button id="pointsRefresh" class="btn secondary" style="width:auto">REFRESH</button></div><div><input id="pointsPerUsd" class="field" type="number" min="0.01" step="0.01" placeholder="Points per $1"></div><button id="pointsSaveSettings" class="btn">SAVE POINTS SETTINGS</button><div id="pointsSettingsMsg" class="status"></div></div><div class="card"><h2>Points Gift Catalog</h2><div class="muted">Choose any active WIN gift and set how many Points it costs.</div><div id="pointsCatalogAdmin"></div></div><h2>Points Gift Requests</h2><div id="pointsRedemptionsAdmin"></div>`;
