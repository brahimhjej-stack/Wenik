...
new_mount = r'''function mountAreaSearch(selectId,inputId,menuId,onSelect,initial=''){const sel=$(selectId);if(!sel)return;ensureWenikPartnerDiscoveryStyles();sel.style.display='none';let wrap=document.getElementById(inputId+'Wrap'),input=document.getElementById(inputId),menu=document.getElementById(menuId);if(!wrap){wrap=document.createElement('div');wrap.id=inputId+'Wrap';wrap.className='wenikAreaSearchWrap';input=document.createElement('input');input.id=inputId;input.className='wenikAreaSearchInput';input.placeholder='Search area…';input.autocomplete='off';const chev=document.createElement('span');chev.className='wenikAreaSearchChevron';chev.textContent='⌄';menu=document.createElement('div');menu.id=menuId;menu.className='wenikAreaSearchMenu hidden';wrap.append(input,chev);document.body.appendChild(menu);sel.parentNode.insertBefore(wrap,sel)}
