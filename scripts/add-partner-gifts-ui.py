...
'''
    ...
    js_anchor = "$ ('logoutBtn')" if False else "$('logoutBtn').onclick=async()=>{stopCamera();await sb.auth.signOut();showLogin()};"
    assert js_anchor in p, 'partner JS logout anchor missing'
    js = r'''
async function loadGifts(){
  try{const list=await rpc('partner_my_gifts');$('giftList').innerHTML=(list||[]).map(g=>'<div class="card"><div><b>'+String(g.name||'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]||c))+'</b><div class="muted">'+String(g.description||'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]||c))+'</div><div class="muted">Status: '+String(g.approval_status||'pending').toUpperCase()+'</div></div>'+(g.approval_status==='approved'?'':'<button class="btn secondary" data-delete-gift="'+g.id+'">DELETE</button>')+'</div>').join('')||'<div class="muted" style="margin-top:16px">No gifts added yet.</div>';document.querySelectorAll('[data-delete-gift]').forEach(b=>b.onclick=async()=>{if(!confirm('Delete this gift?'))return;try{await rpc('partner_delete_gift',{p_gift_id:b.dataset.deleteGift});await loadGifts()}catch(e){$('giftMsg').className='error';$('giftMsg').textContent=e.message}})}catch(e){$('giftMsg').className='error';$('giftMsg').textContent=e.message}}
