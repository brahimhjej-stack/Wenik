...
function Home(){
  const [screen,setScreen]=useState<'home'|'partners'>('home');
  ...
  if(screen==='partners') return <View style={{flex:1}}><Pressable onPress={()=>setScreen('home')} style={{backgroundColor:'#09090d',paddingHorizontal:20,paddingTop:12}}><Text style={{color:'#ef159d',fontWeight:'900'}}>‹ HOME</Text></Pressable><Partners/></View>;
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.page}>
    ...
    <View style={s.grid}>{items.map(([title,sub],i)=><Pressable key={title} style={s.card} onPress={()=>title==='Partners'&&setScreen('partners')}><View style={[s.dot,{backgroundColor:[colors.pink,colors.orange,colors.yellow,colors.purple][i]}]}/><Text style={s.cardTitle}>{title}</Text><Text style={s.cardSub}>{sub}</Text></Pressable>)}</View>
